...
validate_android_aar_symbols() {
    ...
    for abi in armeabi-v7a arm64-v8a x86 x86_64; do
        packaged_lib="$aar_dir/jni/$abi/libvss_rust_client_ffi.so"
        symbol_lib="$symbols_dir/$abi/libvss_rust_client_ffi.so"
        ...
        if [ "$packaged_build_id" != "$symbol_build_id" ]; then
            echo "Error: Android build ID mismatch for $abi/libvss_rust_client_ffi.so: packaged=$packaged_build_id symbols=$symbol_build_id"
            rm -rf "$tmp_dir"
            exit 1
        fi
    done
